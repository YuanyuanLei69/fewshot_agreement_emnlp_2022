...
source_disagree_train_dataset = custom_dataset(source_disagree_train)
source_neutral_train_dataset = custom_dataset(source_neutral_train)
source_agree_train_dataset = custom_dataset(source_agree_train)
...
vocabulary = ["so", "oh", "well", "just", "and", "because", "though", "as well", "if", "then", "thus", "unless", "seems", "also", "you", "uh", "still", "seriously", "what about", "how about",
              "in my view", "in my opinion", "after all", "only", "as long as", "I think", "I see", "I know", "in fact", "so that", "even", "I mean", "cause", "at least", "wonder", "anyway",
              "my perspective", "we should", "what if", "should be", "I'm afraid", "for example", "for instance", "conclusion", "summarize", "consequence", "for this reason", "as a result",
              "yes", "correct", "agree", "accept", "support", "true", "like", "good", "exactly", "ok", "okay", "right", "clear", "sure", "thanks", "believe", "of course", "make sense", "as well",
              "favor", "favoring", "too", "either", "can", "reasonable", "exact", "certainly", "I believe", "admit", "clearly", "make sure", "prove", "pretty", "thank", "definitely", "consent",
              "obvious", "same", "yeah", "absolutely", "convincing", "clarify", "point out", "explain", "enough", "completely", "acknowledge", "claim", "it seems to me", "clean up", "fully",
              "help", "reference", "source", "better", "always", "well said", "accurate", "consensus", "explanation", "acclaim", "adequate", "thoughtful", "handy", "pleased", "sufficient",
              "peaceful", "enhance", "appreciate", "honor", "friend", "affirm" ,"trusted", "stimulate", "simple", "hopeful", "believable", "nice", "confirm", "progress", "fine", "strong",
              "rational", "perfect", "impress", "suitable", "decisive", "motivate", "interesting", "respect", "achieve", "adorable", "reaffirm", "succeed", "helpful", "preferable", "satisfied",
              "confident", "advantage", "encourage", "truthful", "satisfy", "steady", "effective", "success", "benefit", "useful", "relevant", "realistic", "agreeable", "ease", "standout", "generous",
              "beneficial", "capable", "insight", "doubtless", "liking", "grateful", "outperform", "reward", "coherence", "easy", "improve", "suffice", "fans", "consistent", "relevance",  "supportive",
              "logical", "qualified", "ideal", "proper", "glad", "concise", "best", "proving", "gain", "approval", "tidy", "authentic", "great", "reliable", "cool", "faithful",
              "no", "not", "never", "nothing", "however", "but", "doesn't", "don't", "isn't", "yet", "none", "hate", "false", "wrong", "doubt", "disagree", "how can", "I don't think", "missing", "how come",
              "does not", "do not", "aren't", "are not", "is not", "instead", "?", "against", "missed", "unlikeness", "didn't", "did not", "stop", "disfavoring", "why", "neither", "nor", "can not", "can't",
              "harm", "hurt", "harmful", "lie", "worse", "unreasonable", "apologize", "fault", "nobody", "I don't believe", "deny", "joke", "kidding", "or not", "confuse", "confusion", "were not", "weren't",
              "hard", "few", "forget", "misunderstanding", "who knows", "sorry", "stupid", "problem", "dissent", "refuse", "incorrect", "sad", "not true", "no evidence", "do you mean", "I don't know", "I don't see",
              "you don't know", "we don't have", "I am wondering", "you don't understand", "offend", "nope", "oppose", "issue", "while", "limited", "criticize", "criticism", "ridiculous", "ludicrous", "difficult",
              "angry", "damage", "hedge", "miss",  "clumsy", "pity",  "kill", "ineffective",   "offense", "impatience", "unbearable", "strange", "lack",
              "afraid", "complex", "annoy", "foolish", "cons",  "lied",  "noisy", "mess",  "disappoint", "unfair", "junk", "stupidity", "pretend",
              "blurring", "opponent", "diss",  "inefficiency", "frighten",  "broke", "skeptical", "hopeless",  "inequality", "deject", "restrict",  "concern", "complain",
              "controversial", "inconsistence", "disregard", "mad",   "guilty", "collapse",  "slow",  "degenerate", "anxious", "abnormal",  "unfaithful", "contend", "error",
              "betraying", "denial", "badly", "incomplete", "impossible", "degrade", "ashamed", "accuse", "accusation", "lacking", "bother", "failure", "bothering",
              "corrupt", "bias",  "troublesome",   "insane", "absence", "cheater", "uncertain", "burden", "abusive", "unavailable",   "defect", "aggressive", "dumb",
              "concerned", "deaf",  "poor",  "ignore", "cheating",  "crack", "unbelieve", "confess", "imbalance", "sucked", "unacceptable",  "conflict",  "impossibly",
              "distrust",  "silly", "pitiful", "terrible",  "dislike", "break", "die",   "darken", "unauthentic",  "dispute", "odd",   "hating", "puzzled",
              "disbelieve", "mistake", "clueless",  "idiot", "illogically",   "insignificant", "uncomfortable", "doubtful",  "ill",   "shit",  "bored", "pain",  "contradiction",
              "overwhelm", "boring", "inconsistent",  "delay", "averse", "allergic",  "stubborn",  "coarse", "conflicted", "anger", "painful", "arrogant",  "object",
              "acerbic", "ignorance", "struggle",  "refused", "disadvantage",  "confusing", "complicated",   "disapproval",   "cheat", "unconvincing",  "weak",  "stuck", "annoying",
              "objection", "anti",  "inferior",  "contradict", "mistrust",  "reject", "drop",  "attack", "adverse", "liar",  "concede", "rejected",  "disapprove",
              "fuck",  "fail",  "ignorant",  "excuse", "mistaken",  "cheated", "abuse", "afflict", "unconfirmed",   "suck",  "aggravate", "lacked", "betray",
              "idiotic", "anxiety", "loss",  "hateful", "bait",  "punish", "banish", "illness", "stupidly",  "ambiguous", "idiocy", "dangerous", "skeptic",
              "injury", "illogic", "illogical", "blur",  "contradictory",  "trouble", "disgust",
              "maybe", "probably", "would", "could", "rather", "although", "really", "actually", "wondering", "possibly", "essentially", "anyway", "somewhat", "somehow", "I suppose", "perhaps", "'d like",
              "would like", "would rather", "whatever", "may", "wouldn't", "might", "generally", "personally"]



vocabulary_encode = []
for voc in vocabulary:
    enc = tokenizer.encode(voc)[1:(len(tokenizer.encode(voc)) - 1)]
    vocabulary_encode.extend(enc)
print(vocabulary_encode)
...
source_disagree_train_dataloader, source_neutral_train_dataloader, source_agree_train_dataloader = form_dataloaders(source_disagree_train_dataset, source_neutral_train_dataset, source_agree_train_dataset, batch_size=batch_size, shuffle_boolen=False, pop_boolen=True)
num_train_steps = num_epochs * min(len(source_disagree_train_dataloader), len(source_neutral_train_dataloader), len(source_agree_train_dataloader))
...
for epoch_i in range(num_epochs):
    ...
    source_disagree_train_dataloader, source_neutral_train_dataloader, source_agree_train_dataloader = form_dataloaders(source_disagree_train_dataset, source_neutral_train_dataset, source_agree_train_dataset, batch_size=batch_size, shuffle_boolen=True, pop_boolen=True)

    for i in range(min(len(source_disagree_train_dataloader), len(source_neutral_train_dataloader), len(source_agree_train_dataloader))):
        ...
        if i % (10 * num_epochs) == 0 and not i == 0:
            ...
            print('  Batch {:>5,}  of  {:>5,}.    Elapsed: {:}.    Training Loss Average: {:.3f}'.format(i, min(len(source_disagree_train_dataloader), len(source_neutral_train_dataloader), len(source_agree_train_dataloader)), elapsed, avg_train_loss))
            ...
            if macro_F > best_macro_F:
                torch.save(fewshot_model.state_dict(), './saved_models/fewshot_model_add_loss_best.ckpt')
        ...
        if query_idx == len(source_agree_train_dataloader):
            query_idx = 0

        support_quote_input_ids, support_quote_token_type_ids, support_quote_attention_mask, support_response_input_ids, support_response_token_type_ids, support_response_attention_mask, support_label = form_sets_threeclasses(source_disagree_train_dataloader, source_neutral_train_dataloader, source_agree_train_dataloader, support_idx)
        query_quote_input_ids, query_quote_token_type_ids, query_quote_attention_mask, query_response_input_ids, query_response_token_type_ids, query_response_attention_mask, query_label = form_sets_threeclasses(source_disagree_train_dataloader, source_neutral_train_dataloader, source_agree_train_dataloader, query_idx)
        support_quote_input_ids, support_quote_token_type_ids, support_quote_attention_mask, support_response_input_ids, support_response_token_type_ids, support_response_attention_mask, support_label = support_quote_input_ids.to(device), support_quote_token_type_ids.to(device), support_quote_attention_mask.to(device), support_response_input_ids.to(device), support_response_token_type_ids.to(device), support_response_attention_mask.to(device), support_label.to(device)
        query_quote_input_ids, query_quote_token_type_ids, query_quote_attention_mask, query_response_input_ids, query_response_token_type_ids, query_response_attention_mask, query_label = query_quote_input_ids.to(device), query_quote_token_type_ids.to(device), query_quote_attention_mask.to(device), query_response_input_ids.to(device), query_response_token_type_ids.to(device), query_response_attention_mask.to(device), query_label.to(device)
        ...
        with torch.backends.cudnn.flags(enabled=False):
            relation_scores, few_shot_loss, \
            support_disagree_quote_words_embeddings, support_disagree_response_words_embeddings, \
            support_neutral_quote_words_embeddings, support_neutral_response_words_embeddings, \
            support_agree_quote_words_embeddings, support_agree_response_words_embeddings, \
            query_quote_words_embeddings, query_response_words_embeddings = fewshot_model(support_quote_input_ids, support_quote_token_type_ids, support_quote_attention_mask, support_response_input_ids, support_response_token_type_ids, support_response_attention_mask, support_label,
                                                                                          query_quote_input_ids, query_quote_token_type_ids, query_quote_attention_mask, query_response_input_ids, query_response_token_type_ids, query_response_attention_mask, query_label)


        if i % num_epochs == 0 and not i == 0:

            rational_gradient_loss = 0

            for l in range(relation_scores.shape[0]):

                output = relation_scores[l, query_label[l]]

                query_quote_words_grad = torch.autograd.grad(output, query_quote_words_embeddings, create_graph=True)[0][l, :, :]  # 128 * 768
                query_response_words_grad = torch.autograd.grad(output, query_response_words_embeddings, create_graph=True)[0][l, :, :]

                # l2 norm as importance level

                query_quote_words_grad = torch.norm(query_quote_words_grad, dim=-1) # 128
                query_response_words_grad = torch.norm(query_response_words_grad, dim=-1)

                query_quote_words_grad = torch.mul(query_quote_words_grad, query_quote_attention_mask[l,:]) # delete grad norm on PAD
                query_response_words_grad = torch.mul(query_response_words_grad, query_response_attention_mask[l, :])

                query_quote_words_grad = query_quote_words_grad / query_quote_words_grad.sum()
                query_response_words_grad = query_response_words_grad / query_response_words_grad.sum()

                # lexicon-based regularization loss

                query_quote_rationals = torch.as_tensor([int(i in vocabulary_encode) for i in query_quote_input_ids[l,:]], dtype=float).cuda()
                query_quote_rationals_gradient_loss = torch.mul(query_quote_rationals, query_quote_words_grad).sum() #/ query_quote_rationals.sum()
                query_response_rationals = torch.as_tensor([int(i in vocabulary_encode) for i in query_response_input_ids[l, :]], dtype=float).cuda()
                query_response_rationals_gradient_loss = torch.mul(query_response_rationals, query_response_words_grad).sum() #/ query_response_rationals.sum()
                rational_gradient_loss = rational_gradient_loss + query_quote_rationals_gradient_loss + query_response_rationals_gradient_loss


            rational_gradient_loss = rational_gradient_loss / relation_scores.shape[0]

            batch_loss = few_shot_loss - 1.0 * rational_gradient_loss
            print("few_shot_loss, rational_gradient_loss, batch_loss", few_shot_loss, rational_gradient_loss, batch_loss)
            total_train_loss += few_shot_loss.item()
...
fewshot_model.load_state_dict(torch.load("./saved_models/fewshot_model_add_loss_best.ckpt", map_location=device))
