    ...
    for i in range(min(len(source_disagree_train_dataloader), len(source_neutral_train_dataloader), len(source_agree_train_dataloader))):
        ...
        if i % (10 * num_epochs) == 0 and not i == 0:
            ...
            if macro_F > best_macro_F:
                torch.save(fewshot_model.state_dict(), './saved_models/fewshot_model_best.ckpt')
        ...
        if query_idx == min(len(source_disagree_train_dataloader), len(source_neutral_train_dataloader), len(source_agree_train_dataloader)):
            query_idx = 0
...
fewshot_model.load_state_dict(torch.load("./saved_models/fewshot_model_best.ckpt", map_location=device))
